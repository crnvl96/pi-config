...
function countOccurrences(content: string, oldText: string, wholeLines: boolean): number {
  ...
  while (index !== -1) {
    if (!wholeLines || isWholeLineBoundary(fuzzyContent, index, fuzzyOldText.length, fuzzyOldText))
      count++;
...
function parseRowScript(text: string): RawFileScript[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (raw.startsWith("@")) {
      ...
      if (delMatch) {
        ...
        if (startLine < 1 || endLine < startLine)
          throw new Error(`Line ${lineNumber}: invalid inclusive delete range ${trimmed}.`);
    ...
    throw new Error(
      `Line ${lineNumber}: invalid row script line. Every non-empty row must start with [filename], @, +, or -.`,
    );
  }
...
function applyReplaceOperation(
  ...
  try {
    return applyEditsToNormalizedContent(content, primaryEdits, path, { requireWholeLines: true })
      .newContent;
  } catch (err) {
    ...
    return applyEditsToNormalizedContent(content, fallbackEdits, path, { requireWholeLines: true })
      .newContent;
  }
}
